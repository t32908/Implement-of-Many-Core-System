...
void Testbench::do_GaussianB_input() {
  ...
  data.uint = width;
  initiator.write_to_socket(Gaussian_MM_BASE + Gaussian_B_FILTER_R_ADDR, mask, data.uc, 4);
  data.uint = height;
  initiator.write_to_socket(Gaussian_MM_BASE + Gaussian_B_FILTER_R_ADDR, mask, data.uc, 4);
...
void Testbench::do_GaussianB_output(){
  ...
  for (int y = 0; y != height; ++y) {
    for (int x = 0; x != width; ++x) {
      initiator.read_from_socket(Gaussian_MM_BASE + Gaussian_B_FILTER_RESULT_ADDR, mask, data.uc, 4);
