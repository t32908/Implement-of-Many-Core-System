...
Testbench::Testbench(sc_module_name n)
  ...
  SC_THREAD(do_GaussianB_input);
  sensitive << i_clk.pos();
  SC_THREAD(do_GaussianB_output);
  sensitive << i_clk.pos();
  dont_initialize();
}
...
void Testbench::do_GaussianB_input() {
...
void Testbench::do_GaussianB_output(){
  for (int y = 0; y != height; ++y) {
    for (int x = 0; x != width; ++x) {
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = i_result_red.read();
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = i_result_green.read();
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = i_result_blue.read();
        wait();
    }
  }
  sc_stop();
